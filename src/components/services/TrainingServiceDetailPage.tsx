...
import { getTrainingServiceBySlug, trainingServicesData } from '@/data/trainingServicesData';

const TrainingServiceDetailPage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const service = slug ? getTrainingServiceBySlug(slug) : undefined;

  if (!service) {
    return <Navigate to="/training-services" replace />;
  }

  const currentIndex = trainingServicesData.findIndex(s => s.slug === slug);
  const prevService = currentIndex > 0 ? trainingServicesData[currentIndex - 1] : null;
  const nextService = currentIndex < trainingServicesData.length - 1 ? trainingServicesData[currentIndex + 1] : null;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-20">
        <div className="bg-secondary/30 border-b border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4">
            <nav className="flex items-center gap-2 text-sm">
              ...
              <Link to="/training-services" className="text-muted-foreground hover:text-foreground transition-colors">Training Services</Link>
              <ChevronRight className="w-4 h-4 text-muted-foreground" />
        ...
        <section className="bg-gradient-to-br from-primary/5 via-background to-accent/5 py-20 md:py-28">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="max-w-3xl">
              <span className="text-sm font-medium text-accent mb-4 block">Training & Capacity Development</span>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                ...
                <Link to="/training-services" className="inline-flex items-center gap-2 bg-secondary text-secondary-foreground px-6 py-3 rounded-lg font-medium hover:bg-secondary/80 transition-colors">
                  <ArrowLeft className="w-4 h-4" /> All Training Programs
                </Link>
              </div>
            </div>
          </div>
        ...
        <section className="py-12 border-t border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="flex justify-between items-center">
              {prevService ? (
                <Link to={`/training-services/${prevService.slug}`} className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors group">
              ...
              {nextService ? (
                <Link to={`/training-services/${nextService.slug}`} className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors group">
        ...
        <section id="contact" className="py-20 md:py-24 bg-primary text-primary-foreground">
          <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Ready to upskill your teams?</h2>
            <p className="text-lg opacity-90 mb-8">
              Let's discuss how our {service.shortTitle.toLowerCase()} programs can build your team's capabilities.
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link to="/training-services" className="inline-flex items-center gap-2 border border-primary-foreground/30 px-6 py-3 rounded-lg font-medium hover:bg-primary-foreground/10 transition-colors">
...
export default TrainingServiceDetailPage;
