...
import { getInfraServiceBySlug, infrastructureServicesData } from '@/data/infrastructureServicesData';

const InfraServiceDetailPage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const service = slug ? getInfraServiceBySlug(slug) : undefined;

  if (!service) {
    return <Navigate to="/infrastructure-services" replace />;
  }

  const currentIndex = infrastructureServicesData.findIndex(s => s.slug === slug);
  const prevService = currentIndex > 0 ? infrastructureServicesData[currentIndex - 1] : null;
  const nextService = currentIndex < infrastructureServicesData.length - 1 ? infrastructureServicesData[currentIndex + 1] : null;
        ...
        <div className="bg-secondary/30 border-b border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4">
            <nav className="flex items-center gap-2 text-sm">
              ...
              <Link to="/infrastructure-services" className="text-muted-foreground hover:text-foreground transition-colors">Infrastructure Services</Link>
              <ChevronRight className="w-4 h-4 text-muted-foreground" />
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="max-w-3xl">
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                ...
                <Link to="/infrastructure-services" className="inline-flex items-center gap-2 bg-secondary text-secondary-foreground px-6 py-3 rounded-lg font-medium hover:bg-secondary/80 transition-colors">
                  <ArrowLeft className="w-4 h-4" /> All Infrastructure Services
                </Link>
              </div>
            </div>
          </div>
        ...
        <section className="py-12 border-t border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="flex justify-between items-center">
              ...
              {nextService ? (
                <Link to={`/infrastructure-services/${nextService.slug}`} className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors group">
        ...
        <section id="contact" className="py-20 md:py-24 bg-primary text-primary-foreground">
          <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link to="/infrastructure-services" className="inline-flex items-center gap-2 border border-primary-foreground/30 px-6 py-3 rounded-lg font-medium hover:bg-primary-foreground/10 transition-colors">
...
export default InfraServiceDetailPage;
