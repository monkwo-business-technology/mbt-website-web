...
import { getTalentServiceBySlug, talentServicesData } from '@/data/talentServicesData';

const TalentServiceDetailPage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const service = slug ? getTalentServiceBySlug(slug) : undefined;

  if (!service) {
    return <Navigate to="/talent-services" replace />;
  }

  const currentIndex = talentServicesData.findIndex(s => s.slug === slug);
  const prevService = currentIndex > 0 ? talentServicesData[currentIndex - 1] : null;
  const nextService = currentIndex < talentServicesData.length - 1 ? talentServicesData[currentIndex + 1] : null;
        ...
        <div className="bg-secondary/30 border-b border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4">
            <nav className="flex items-center gap-2 text-sm">
              ...
              <Link to="/talent-services" className="text-muted-foreground hover:text-foreground transition-colors">Talent Outsourcing</Link>
              <ChevronRight className="w-4 h-4 text-muted-foreground" />
        ...
        <section className="bg-gradient-to-br from-primary/5 via-background to-accent/5 py-20 md:py-28">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="max-w-3xl">
              <span className="text-sm font-medium text-accent mb-4 block">Talent Outsourcing</span>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                ...
                <Link to="/talent-services" className="inline-flex items-center gap-2 bg-secondary text-secondary-foreground px-6 py-3 rounded-lg font-medium hover:bg-secondary/80 transition-colors">
                  <ArrowLeft className="w-4 h-4" /> All Talent Services
                </Link>
              </div>
            </div>
          </div>
        ...
        <section className="py-12 border-t border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="flex justify-between items-center">
              {prevService ? (
                <Link to={`/talent-services/${prevService.slug}`} className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors group">
              ...
              {nextService ? (
                <Link to={`/talent-services/${nextService.slug}`} className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors group">
        ...
        <section id="contact" className="py-20 md:py-24 bg-primary text-primary-foreground">
          <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
            ...
            <p className="text-lg opacity-90 mb-8">
              Let's discuss how our {service.shortTitle.toLowerCase()} services can accelerate your projects.
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link to="/talent-services" className="inline-flex items-center gap-2 border border-primary-foreground/30 px-6 py-3 rounded-lg font-medium hover:bg-primary-foreground/10 transition-colors">
                Explore All Talent Services
              </Link>
            </div>
          </div>
        </section>
...
export default TalentServiceDetailPage;
