...
import { getDataServiceBySlug, dataServicesData } from '@/data/dataServicesData';

const DataServiceDetailPage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const service = slug ? getDataServiceBySlug(slug) : undefined;

  if (!service) {
    return <Navigate to="/data-services" replace />;
  }

  const currentIndex = dataServicesData.findIndex(s => s.slug === slug);
  const prevService = currentIndex > 0 ? dataServicesData[currentIndex - 1] : null;
  const nextService = currentIndex < dataServicesData.length - 1 ? dataServicesData[currentIndex + 1] : null;
        ...
        <div className="bg-secondary/30 border-b border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4">
            <nav className="flex items-center gap-2 text-sm">
              ...
              <Link to="/data-services" className="text-muted-foreground hover:text-foreground transition-colors">
                Data Services
              </Link>
              <ChevronRight className="w-4 h-4 text-muted-foreground" />
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {service.features.map((feature, index) => (
                <div
                  key={index}
                  className="bg-card border border-border/50 rounded-xl p-6 hover:border-accent/30 transition-all duration-300 hover:shadow-lg"
                >
        ...
        <section className="py-20 md:py-24 bg-secondary/30">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="mb-12">
              ...
              <p className="text-muted-foreground max-w-2xl">
                A proven methodology to deliver successful data solutions.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-muted-foreground mb-8">
                  Partner with Monkwo for data solutions that deliver measurable business value.
                </p>
        ...
        <section className="py-12 border-t border-border/50">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="flex justify-between items-center">
              {prevService ? (
                <Link
                  to={`/data-services/${prevService.slug}`}
              ...
              {nextService ? (
                <Link
                  to={`/data-services/${nextService.slug}`}
                  className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors group"
                >
        ...
        <section id="contact" className="py-20 md:py-24 bg-primary text-primary-foreground">
          <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
            ...
            <p className="text-lg opacity-90 mb-8">
              Let's discuss how our {service.shortTitle.toLowerCase()} solutions can drive value for your business.
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link
                to="/data-services"
                className="inline-flex items-center gap-2 border border-primary-foreground/30 px-6 py-3 rounded-lg font-medium hover:bg-primary-foreground/10 transition-colors"
              >
...
export default DataServiceDetailPage;
